...
const CreateSchema = z.object({
  ...
  materialAlternatives: z.array(z.string().min(1).max(20)).max(9).optional(),
  materialNotes: z.string().max(500).optional().nullable(),
  partColors: z.array(z.string().regex(/^#[0-9a-fA-F]{6}$/)).max(20),
  colorMatters: z.boolean().optional(),
...
export async function POST(req: Request) {
  ...
  const job = await prisma.job.create({
    data: {
      ...
      material: d.material,
      materialAlternatives: JSON.stringify(d.materialAlternatives ?? []),
      materialNotes: d.materialNotes?.trim() || null,
      partColors: JSON.stringify(d.partColors ?? []),
      colorMatters: d.colorMatters ?? false,
